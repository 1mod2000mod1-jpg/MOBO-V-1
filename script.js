...
function initializeSocket() {
    try {
        // 🚨 التصحيح 1: تحديد رابط الخادم يدوياً للاستضافة على Render
        const RENDER_URL = "https://mobi-1.onrender.com"; 
...
function setupSocketListeners() {
    // 🚨 التصحيح 2: يمنع خطأ TypeError: Cannot read properties of null (reading 'on')
    // نتحقق أولاً من أن كائن الاتصال قد تم إنشاؤه بنجاح
    if (!socket) {
        console.error("❌ فشل تهيئة كائن الاتصال Socket.io، تم منع خطأ القراءة.");
        return; 
    }
...
// 🚨 التصحيح 3: تم حذف الأكواد المشوهة والمكررة في نهاية الملف.
console.log(`
╔═══════════════════════════════════════════════════════╗
